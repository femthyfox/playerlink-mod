...
3. **Whitelist-only candidates** — the candidate list is built from the
   server's vanilla `whitelist.json` (the same one accepted by `/whitelist
   add`).
...
└── resources/
    ...
    └── assets/playerlink/lang/en_us.json
```
...
> <https://maven.createmod.net/> and update `create_version`,
> `flywheel_version`, `registrate_version` if needed.
